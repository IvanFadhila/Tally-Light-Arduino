...
#include <avr/wdt.h>
//---------------------------------------------------------------------------- Config LCD & NRF
LiquidCrystal_I2C lcd(0x27,16,2);  //Address 0x27 LCD 1602
...
void Check_pin(){
  ...
  switch (MODE){
    ...
    lcd.setCursor (15,1); 
    if (bitRead(PIN[0],0)==1)  {
      LastPrev=1;
      lcd.print("1");
    }
    if (bitRead(PIN[0],2)==1)  {
      LastPrev=2;
      lcd.print("2");
    }
    if (bitRead(PIN[0],4)==1)  {
      LastPrev=3;
      lcd.print("3");
    }
    if (bitRead(PIN[1],0)==1)  {
      LastPrev=4;
      lcd.print("4");
    }
    if (bitRead(PIN[1],2)==1)  {
      LastPrev=5;
      lcd.print("5");
    }
    if (bitRead(PIN[1],4)==1)  {
      LastPrev=6;
      lcd.print("6");
    }
